...
export class FaceMeshFaceGeometry extends BufferGeometry {
  ...
  setUvs() {
    ...
    for (let j = 0; j < 468; j++) {
      this.maskUVs[j * 2] = this.flipped
        ? 1 - texCoords[j][0]
        : texCoords[j][0];
      this.maskUVs[j * 2 + 1] = 1 - texCoords[j][1];
    }
    this.getAttribute("maskUV").needsUpdate = true;
  }
  ...
  getInfoFrom3Pts(p0, p1, p2) {
    this.p0.fromArray(p0);
